...
function setLanguage(lang) {
    ...
    infoIcons.forEach(icon => {
        ...
        // --- DOM Element References (Continued - ensure all are captured) ---
        // (Many were in the first third, add any missed or new ones if the HTML structure changed significantly)
        // For dynamic elements within tables, references are often handled during row creation.
        const allDataInputs = document.querySelectorAll('.data-input, .data-input-dim, .data-input-pressure-config'); // Combine for easier generic listeners if needed
        
        // --- Function Definitions (Continued) ---
        
        /**
         * Set Language Function (Completion)
         */
        function setLanguage(lang) {
            if (!translations[lang]) {
                console.warn(`Translations for language "${lang}" not found. Defaulting to 'en'.`);
            ...
            document.querySelectorAll('[data-translate-key]').forEach(element => {
                ...
                if (translation != null) {
                    if (key === 'disclaimer' || (element.innerHTML.includes('<strong>') && typeof translation === 'string' && translation.includes('<strong>'))) {
                        ...
                        } else if (element.placeholder && typeof translation === 'string' && !key.startsWith('explanation_')) { // Avoid using complex explanations as placeholders
                             element.placeholder = translation;
                        }
                        ...
                        const valueKey = `option_${element.parentElement.id}_${element.value}`;
                         if(translations[lang][valueKey]) {
                            element.textContent = translations[lang][valueKey];
                        } else if (translations[lang][element.dataset.translateKey]) { // if option itself has a key
                    ...
                    const listElement = document.querySelector(`[data-translate-key="${key}"]`);
                    if (listElement) {
                        listElement.innerHTML = `<li>${translations[lang].list_unavailable || "Content not available."}</li>`;
                    }
                }
            });
        
            // Update appliance table rows
            tableBody.querySelectorAll('.appliance-row').forEach(row => updateApplianceRowTranslations(row, lang));
        
            // Update duct system table rows (NEW)
            ductSystemTableBody.querySelectorAll('.duct-system-row').forEach(row => updateDuctSystemRowTranslations(row, lang));
        
        
            // Update general select options (e.g. hoodType, cookingDutyLevel, ductMaterial (if it were static), outletType)
            // This is partially handled by data-translate-key on options, but dynamic selects might need more.
            // Example for a static select like outletType:
            ...
            infoIcons.forEach(icon => {
                ...
                if (titleKey) icon.title = translations[lang][titleKey] || "Click for explanation";
            });
        ...
        /**
         * Updates translations for a single appliance row.
        ...
        function updateApplianceRowTranslations(row, lang) {
            ...
            if (equipSelect) {
                const currentEquipValue = equipSelect.value;
                equipSelect.innerHTML = ''; // Clear existing options
                ...
                for (const groupKey in equipOptgroups) {
                    ...
                    equipOptgroups[groupKey].forEach(equipKey => {
                        if (SENSIBLE_FACTORS.hasOwnProperty(equipKey)) { // Ensure factor exists
            ...
            const gasSelect = row.querySelector('.gas-type');
            if (gasSelect) { /* ... (translation as in first third) ... */
                const currentGasValue = gasSelect.value;
                gasSelect.innerHTML = `
                    <option value="natural_pt">${translations[lang].gas_type_natural || "Natural Gas"}</option>
                    <option value="propane">${translations[lang].gas_type_propane || "Propane"}</option>
            ...
            const unitSelect = row.querySelector('.gas-unit');
            if (unitSelect) { /* ... (translation as in first third) ... */
                const currentUnitValue = unitSelect.value;
                unitSelect.innerHTML = `
                    <option value="m3h">${translations[lang].gas_unit_m3h || "m³/h"}</option>
                    <option value="kgh">${translations[lang].gas_unit_kgh || "kg/h"}</option>
            ...
            const deleteBtn = row.querySelector('.delete-btn');
            if (deleteBtn) deleteBtn.textContent = translations[lang].delete_button || "Delete";
            const nameInput = row.querySelector('.appliance-name');
            if (nameInput) nameInput.placeholder = translations[lang].th_appliance_name || 'Appliance Name';
        }
        
        /**
         * Updates translations for select elements within a duct system row (NEW)
         * @param {HTMLElement} row - The table row element for a duct system component.
         * @param {string} lang - The language code.
         */
        function updateDuctSystemRowTranslations(row, lang) {
            // Example for a material select if it exists in the row
            const materialSelect = row.querySelector('.duct-material-select');
            if (materialSelect) {
                const currentMaterial = materialSelect.value;
                materialSelect.innerHTML = ''; // Clear
                Object.keys(DUCT_ROUGHNESS).forEach(matKey => {
                    const optionText = translations[lang][`pressure_duct_mat_${matKey}`] || matKey.replace(/_/g, ' ');
                    const option = new Option(optionText, matKey);
                    materialSelect.add(option);
                });
                materialSelect.value = currentMaterial;
            }
        
            // Example for a fitting type select
            ...
            if (fittingTypeSelect) {
                ...
                fittingTypeSelect.innerHTML = `<option value="" data-translate-key="option_placeholder_select">${translations[lang].option_placeholder_select || "Select..."}</option>`; // Placeholder
                Object.keys(FITTING_K_FACTORS).forEach(fitKey => {
                    // Exclude functions from being listed directly as simple options if K is dynamic
                    // Corrected logic: Use the same filtering as in addFittingRowUI
                    // i.e., exclude system-level items not meant for user selection in the duct layout table.
                    if (fitKey !== 'hood_entry_assumed' && !fitKey.startsWith('outlet_')) {
                        ...
                        const option = new Option(optionText, fitKey);
                        fittingTypeSelect.add(option);
                    }
                });
                fittingTypeSelect.value = currentFitting;
            }
            // Translate other labels/placeholders within the row if they have data-translate-key
            row.querySelectorAll('[data-translate-key]').forEach(el => {
                ...
                if (translations[lang][key]) {
                    ...
                    else if (el.tagName !== 'SELECT' && el.tagName !== 'OPTION') el.textContent = translations[lang][key];
                }
            });
            const removeBtn = row.querySelector('.remove-item-btn');
            if (removeBtn) removeBtn.textContent = translations[lang].remove_button || "Remove";
        }
        
        
        /**
         * Toggles Gas Unit Selector visibility and value based on gas type.
         */
        function toggleGasUnitSelector(gasTypeSelectElement, gasUnitSelectElement) {
            ...
            const currentlyHidden = gasUnitSelectElement.style.display === 'none';
        
            gasUnitSelectElement.style.display = isLPG ? 'inline-block' : 'none';
            if (isLPG && currentlyHidden) {
                gasUnitSelectElement.value = 'kgh';
            } else if (!isLPG) {
                gasUnitSelectElement.value = 'm3h';
            }
        }
        
        /**
         * Adds a new appliance row to the table.
         */
        function addApplianceRow(rowData = null) {
            const row = tableBody.insertRow();
            row.classList.add('appliance-row');
            const lang = currentLang; // Use current language
        
            // Build equipment type select with optgroups
            let equipmentOptionsHTML = '';
            const equipOptgroups = { /* ... (same as in updateApplianceRowTranslations) ... */
                optgroup_fryers: ['fryer_open_pot', 'fryer_tube', 'fryer_pressure', 'fryer'],
                optgroup_grills: ['grill_charbroiler_radiant', 'grill_charbroiler_lava', 'griddle_plate', 'grill_clam_shell', 'grill'],
                optgroup_ranges: ['range_open_burner_gas', 'range_hot_top_electric', 'range_induction', 'wok_range_gas', 'range_top'],
                optgroup_ovens: ['oven_convection', 'oven_deck', 'oven_combi', 'oven_pizza_conveyor', 'oven_rotisserie', 'oven'],
            ...
            const inputClasses = "w-full p-2 border border-gray-300 rounded-md shadow-sm text-sm data-input";
            const selectClasses = inputClasses;
        
            row.innerHTML = `
                <td class="px-2 py-2 whitespace-nowrap"><input type="text" placeholder="${translations[lang].th_appliance_name}" class="${inputClasses} appliance-name"></td>
                <td class="px-2 py-2 whitespace-nowrap"><select class="${selectClasses} equipment-type">${equipmentOptionsHTML}</select></td>
                <td class="px-2 py-2 whitespace-nowrap">
                    <select class="${selectClasses} gas-type">
                ...
                <td class="px-2 py-2 whitespace-nowrap"><input type="number" step="0.01" min="0" placeholder="0.0" class="${inputClasses} gas-usage"></td>
                <td class="px-2 py-2 whitespace-nowrap">
                    <select class="${selectClasses} gas-unit-selector gas-unit" style="display: none;">
                        <option value="m3h">${translations[lang].gas_unit_m3h}</option>
                        <option value="kgh">${translations[lang].gas_unit_kgh}</option>
                    </select>
                </td>
                <td class="px-2 py-2 whitespace-nowrap"><input type="number" step="0.1" min="0" placeholder="0.0" class="${inputClasses} electric-usage"></td>
                <td class="px-2 py-2 whitespace-nowrap"><span class="calculated-value total-power">0.0 kW</span></td>
                <td class="px-2 py-2 whitespace-nowrap"><span class="calculated-value sensible-factor">0.0</span></td>
                <td class="px-2 py-2 whitespace-nowrap"><span class="calculated-value sensible-heat">0.0 kW</span></td>
                <td class="px-2 py-2 whitespace-nowrap"><button class="text-red-600 hover:text-red-800 delete-btn text-sm font-medium">${translations[lang].delete_button}</button></td>
            `;
        
            if (rowData) {
                row.querySelector('.appliance-name').value = rowData.name || '';
                row.querySelector('.equipment-type').value = rowData.equipValue || equipOptgroups.optgroup_other_equip[0]; // Default to 'other'
            ...
            // Add event listeners to all relevant inputs/selects in the new row
            ...
            if (!rowData) performCalculations(); // Recalculate if it's a fresh row
        }
        
        /**
         * Deletes an appliance row from the table.
         * @param {HTMLElement} rowElement - The table row element to delete.
         */
        ...
        /**
         * Calculates airflow based on the Heat Load Method.
         */
        function calculateAirflowByHeatLoad() {
            let totalSensibleHeat = 0;
            applianceDataForComparison = []; // Reset for fresh data
        
            tableBody.querySelectorAll('.appliance-row').forEach(row => {
                const equipmentType = row.querySelector('.equipment-type').value;
                const gasType = row.querySelector('.gas-type').value;
                const gasUsageInput = row.querySelector('.gas-usage');
                const gasUnit = row.querySelector('.gas-unit').value;
                const electricUsageInput = row.querySelector('.electric-usage');
        
                const gasUsage = parseFloat(gasUsageInput.value) || 0;
                const electricUsage = parseFloat(electricUsageInput.value) || 0;
        
                let ncv = 0;
                if (gasType === 'natural_pt') ncv = NCV_VALUES.natural_pt_m3h || 0;
                else if (gasType === 'propane') ncv = (gasUnit === 'kgh') ? (NCV_VALUES.propane_kgh || 0) : (NCV_VALUES.propane_m3h || 0);
                else if (gasType === 'butane') ncv = (gasUnit === 'kgh') ? (NCV_VALUES.butane_kgh || 0) : (NCV_VALUES.butane_m3h || 0);
            ...
            totalSensibleHeat *= diversity; // Apply diversity factor
        ...
        /**
         * Calculates airflow based on the Hood Dimensions Method.
         */
        function calculateAirflowByDimensions() {
            ...
            return { airflowLow: airflow, airflowHigh: airflow, totalSensibleHeat: 0 }; // No sensible heat for this method
        }
        
        /**
         * Master function to perform all calculations based on current method and update UI.
         */
        function performCalculations() {
            try {
                ...
                // Auto-fill duct diameter for the *first segment* if using dynamic duct system and it's empty (NEW logic for segmented system)
                // Also, update the ductSystemLayout array if auto-filling.
                const firstSegmentRow = ductSystemTableBody.querySelector('.duct-segment-row');
                if (firstSegmentRow && !manualAirflowToggle.checked) { // Condition corrected: check toggle, not method string
                    const segmentId = firstSegmentRow.getAttribute('data-id');
                    const segmentInLayout = ductSystemLayout.find(s => s.id === segmentId && s.type === 'segment');
                    if (segmentInLayout) {
                        ...
                        if (shapeSelect && shapeSelect.value === 'round') {
                            ...
                            if (diameterInput && !diameterInput.value && recommendedDiameter > 0) {
                                ...
                                segmentInLayout.diameter = recommendedDiameter; // Update the layout array
                            }
                        }
                    }
                }
                ...
                calculatePressureDrop(); // This will be the complex rewritten function
            } catch (error) {
        ...
        /**
         * Updates the comparison text area based on current calculations.
         */
        function updateComparisonText() {
            ...
            // Calculate heat load results (even if not active method) for comparison
            const heatLoadResultsForComparison = calculateAirflowByHeatLoad(); // Recalculate fresh for comparison
            ...
            // Calculate dimensional results (even if not active method) for comparison
            const dimResultsForComparison = calculateAirflowByDimensions();
            dimAirflowForComparison = dimResultsForComparison.airflowHigh; // Low and High are same for dimensional
            ...
            } else { // hoodDimension method is active
        ...
        /**
         * Calculates Recommended Duct Diameter based on airflow and target velocity.
         */
        function calculateRecommendedDuct(airflowM3H) {
        ...
        /**
         * Toggles visibility of input sections based on selected calculation method.
         */
        function toggleCalculationMethodView() {
            if (currentCalculationMethod === 'heatLoad') {
        ...
        /**
         * Toggles inputs within the dimensional section based on selected hood type.
         */
        function toggleDimensionalInputsBasedOnHoodType() {
            const selectedHoodType = hoodTypeSelect.value;
            if (selectedHoodType === 'eyebrow') {
        ...
        // --- Duct System Layout UI Functions (NEW - Structures) ---
        
        /**
         * Adds a new duct segment row to the UI and layout array.
         */
        function addDuctSegmentRowUI() {
            const segmentId = `segment-${nextDuctElementId++}`;
            const newSegment = {
                id: segmentId,
                type: 'segment',
                length: 1, // Default length
                shape: 'round', // Default shape
                diameter: calculateRecommendedDuct(lastCalculatedAirflowHighM3H) || 200, // Default diameter
                width: 300, // Default for rectangular
                height: 200, // Default for rectangular
                material: 'galvanized' // Default material
            };
            ...
            orderCell.textContent = ductSystemLayout.length; // Simple order for now
        
            // Type Cell
            const typeCell = row.insertCell();
            typeCell.textContent = translations[currentLang].segment_type_segment || "Duct Segment";
        
            // Details/Dimensions Cell - This will be complex with conditional inputs
            const detailsCell = row.insertCell();
            detailsCell.classList.add('space-y-2'); // For stacking inputs
        
            // Shape Selector
            const shapeSelect = document.createElement('select');
            shapeSelect.className = 'duct-shape-select w-full p-1 border border-gray-300 rounded-md shadow-sm text-sm';
            shapeSelect.innerHTML = `
                <option value="round">${translations[currentLang].segment_shape_round || "Round"}</option>
                <option value="rectangular">${translations[currentLang].segment_shape_rect || "Rectangular"}</option>
            `;
            shapeSelect.value = newSegment.shape;
            const shapeWrapper = document.createElement('div');
            shapeWrapper.innerHTML = `<label class="text-xs">${translations[currentLang].segment_shape || "Shape:"}</label>`;
            ...
            diameterInput.step = 10;
            diameterInput.value = newSegment.diameter;
            diameterInput.className = 'duct-diameter-input w-full p-1 border border-gray-300 rounded-md shadow-sm text-sm';
            const diameterWrapper = document.createElement('div');
            diameterWrapper.innerHTML = `<label class="text-xs">${translations[currentLang].segment_diameter || "Diameter (mm):"}</label>`;
            diameterWrapper.appendChild(diameterInput);
            detailsCell.appendChild(diameterWrapper);
        
            // Width & Height Inputs (for rectangular)
            const widthInput = document.createElement('input'); /* ... similar setup ... */
            widthInput.type = 'number'; widthInput.value = newSegment.width; widthInput.className = 'duct-width-input w-full p-1 border ...';
            const widthWrapper = document.createElement('div'); /* ... */ widthWrapper.innerHTML = `<label class="text-xs">${translations[currentLang].segment_width || "Width (mm):"}</label>`; widthWrapper.appendChild(widthInput);
        
            const heightInput = document.createElement('input'); /* ... similar setup ... */
            heightInput.type = 'number'; heightInput.value = newSegment.height; heightInput.className = 'duct-height-input w-full p-1 border ...';
            const heightWrapper = document.createElement('div'); /* ... */ heightWrapper.innerHTML = `<label class="text-xs">${translations[currentLang].segment_height || "Height (mm):"}</label>`; heightWrapper.appendChild(heightInput);
            ...
            // Show/hide diameter vs width/height based on shape
            ...
            toggleSegmentInputs(newSegment.shape); // Initial state
        
            // Parameters Cell (Length, Material)
            const paramsCell = row.insertCell();
            paramsCell.classList.add('space-y-2');
        
            const lengthInput = document.createElement('input');
            lengthInput.type = 'number'; lengthInput.min = 0.1; lengthInput.step = 0.1; lengthInput.value = newSegment.length;
            lengthInput.className = 'duct-length-input w-full p-1 border border-gray-300 rounded-md shadow-sm text-sm';
            const lengthWrapper = document.createElement('div');
            lengthWrapper.innerHTML = `<label class="text-xs">${translations[currentLang].segment_length || "Length (m):"}</label>`;
            lengthWrapper.appendChild(lengthInput);
            paramsCell.appendChild(lengthWrapper);
        
            const materialSelect = document.createElement('select');
            materialSelect.className = 'duct-material-select w-full p-1 border border-gray-300 rounded-md shadow-sm text-sm';
            Object.keys(DUCT_ROUGHNESS).forEach(matKey => {
            ...
            materialWrapper.innerHTML = `<label class="text-xs">${translations[currentLang].segment_material || "Material:"}</label>`;
            ...
            removeBtn.className = 'text-red-600 hover:text-red-800 remove-item-btn text-sm';
            removeBtn.textContent = translations[currentLang].remove_button || "Remove";
            removeBtn.onclick = () => deleteDuctSystemRow(segmentId);
            actionCell.appendChild(removeBtn);
        
            // Add event listeners to inputs to update ductSystemLayout and recalculate
            [diameterInput, widthInput, heightInput, lengthInput, materialSelect].forEach(inputEl => {
                inputEl.addEventListener('change', (e) => {
                    const segment = ductSystemLayout.find(s => s.id === segmentId);
                    if (segment) {
                        if (e.target.classList.contains('duct-diameter-input')) segment.diameter = parseFloat(e.target.value);
                        else if (e.target.classList.contains('duct-width-input')) segment.width = parseFloat(e.target.value);
                        else if (e.target.classList.contains('duct-height-input')) segment.height = parseFloat(e.target.value);
                        else if (e.target.classList.contains('duct-length-input')) segment.length = parseFloat(e.target.value);
                        else if (e.target.classList.contains('duct-material-select')) segment.material = e.target.value;
                        performCalculations();
                    }
                });
                 if(inputEl.type === 'number') inputEl.addEventListener('input', (e) => { /* similar logic for live update */ });
            });
        ...
        /**
         * Adds a new fitting row to the UI and layout array. (Structure - details in next part)
         */
        function addFittingRowUI() {
            const fittingId = `fitting-${nextDuctElementId++}`;
            // Determine upstream dimensions from previous segment if possible
            const lastElement = ductSystemLayout.length > 0 ? ductSystemLayout[ductSystemLayout.length - 1] : null;
            let upstreamDiameter = 200, upstreamWidth = 300, upstreamHeight = 200, upstreamShape = 'round';
            if (lastElement && lastElement.type === 'segment') {
                upstreamShape = lastElement.shape;
                if (lastElement.shape === 'round') upstreamDiameter = lastElement.diameter;
                else {
                    upstreamWidth = lastElement.width;
                    upstreamHeight = lastElement.height;
                }
            }
            ...
            const newFitting = {
                id: fittingId,
                type: 'fitting',
                fittingKey: '', // User will select this
                quantity: 1,
                // For transitions, store relevant dimensions
                d_upstream: upstreamShape === 'round' ? upstreamDiameter : null,
                w_upstream: upstreamShape === 'rectangular' ? upstreamWidth : null,
                h_upstream: upstreamShape === 'rectangular' ? upstreamHeight : null,
                d_downstream: upstreamShape === 'round' ? upstreamDiameter : null, // Default to same, user changes
                w_downstream: upstreamShape === 'rectangular' ? upstreamWidth : null,
                h_downstream: upstreamShape === 'rectangular' ? upstreamHeight : null,
            };
            ...
            // Order Cell
            const orderCell = row.insertCell(); // Will be updated by updateDuctSystemRowOrder
        
            // Type Cell
            const typeCell = row.insertCell();
            typeCell.textContent = translations[currentLang].segment_type_fitting || "Fitting";
            ...
            fittingTypeSelect.className = 'fitting-type-select w-full p-1 border border-gray-300 rounded-md shadow-sm text-sm';
            // Populate fittingTypeSelect (done in updateDuctSystemRowTranslations or here directly)
            ...
            fittingTypeWrapper.innerHTML = `<label class="text-xs">${translations[currentLang].fitting_type || "Fitting Type:"}</label>`;
            fittingTypeWrapper.appendChild(fittingTypeSelect);
            detailsCell.appendChild(fittingTypeWrapper);
        
            // Conditional inputs for transitions (d_upstream, d_downstream, etc.)
            const transitionInputsWrapper = document.createElement('div');
            transitionInputsWrapper.className = 'transition-inputs-wrapper hidden space-y-1 mt-1'; // Hidden by default
            // Add inputs for d_upstream, d_downstream, w_upstream, h_upstream, w_downstream, h_downstream
            // These would be created similar to segment dimension inputs
            // Example for one:
            const dUpstreamInput = document.createElement('input');
            dUpstreamInput.type = 'number'; dUpstreamInput.placeholder = 'Upstream Dia (mm)';
            dUpstreamInput.className = 'fitting-d-upstream w-full p-1 border ...';
            dUpstreamInput.value = newFitting.d_upstream || '';
            transitionInputsWrapper.appendChild(dUpstreamInput); // Simplified, add labels etc.
            // ... add other transition inputs (d_down, w_up, h_up, w_down, h_down)
        
            detailsCell.appendChild(transitionInputsWrapper);
        
        
            // Parameters Cell (Quantity)
            const paramsCell = row.insertCell();
            const quantityInput = document.createElement('input');
            quantityInput.type = 'number'; quantityInput.min = 1; quantityInput.step = 1; quantityInput.value = newFitting.quantity;
            quantityInput.className = 'fitting-quantity-input w-full p-1 border border-gray-300 rounded-md shadow-sm text-sm';
            const quantityWrapper = document.createElement('div');
            quantityWrapper.innerHTML = `<label class="text-xs">${translations[currentLang].fitting_quantity || "Quantity:"}</label>`;
            quantityWrapper.appendChild(quantityInput);
            paramsCell.appendChild(quantityWrapper);
            ...
            // Event listener for fittingTypeSelect to show/hide transition inputs and update data model
            fittingTypeSelect.addEventListener('change', (e) => {
                ...
                if (selectedKey.includes('transition')) {
                    transitionInputsWrapper.classList.remove('hidden');
                    // TODO: Further logic to show round vs rect inputs based on detected upstream/downstream context or user choice
                } else {
                    transitionInputsWrapper.classList.add('hidden');
                }
                performCalculations();
            });
        
            [quantityInput, dUpstreamInput /*, other transition inputs */].forEach(inputEl => {
                if(inputEl) { // Check if element exists
                    inputEl.addEventListener('change', (e) => {
                        const fitting = ductSystemLayout.find(f => f.id === fittingId);
                        if (fitting) {
                            if (e.target.classList.contains('fitting-quantity-input')) fitting.quantity = parseInt(e.target.value);
                            if (e.target.classList.contains('fitting-d-upstream')) fitting.d_upstream = parseFloat(e.target.value);
        ...
        /**
         * Deletes an element (segment or fitting) from the duct system layout.
         * @param {string} elementId - The ID of the element to remove.
         */
        function deleteDuctSystemRow(elementId) {
        ...
        /**
         * Updates the order number in the first cell of each duct system row.
         */
        function updateDuctSystemRowOrder() {
